...
(function () {
	...
	rapps.controller('ClassRegisterCtrl',
			['DataCtx',
				'localStorageService',
				'ngDialog',
				'$rootScope',
				'$state',
				'_m','$stateParams', 'WidgetSrv', ClassRegisterCtrl]);

	function ClassRegisterCtrl(DataCtx, localStorageService, ngDialog, $rootScope, $state, _m, $stateParams, WidgetSrv) {
		var vm = this;
		vm.page = {
		...
		//Load Register
		DataCtx.register.get({id: $stateParams.id}).$promise.then(loadSessions, errLoading);

		vm.studentAttend = studentAttend;
		...
		function studentAttend (id){
			...
			confirm.closePromise.then(function(Data){
				...
				if (Data.value == "cool") {
					//$state.go("home");
					console.log(Data);
					var Register = new DataCtx.register();

					Register.$save({id: id}).then(function(res){
